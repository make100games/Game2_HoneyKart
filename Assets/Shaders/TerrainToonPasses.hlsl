...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    ...
    // Disable blending between layers
...
#endif
{
    ...
    half3 albedo = mixedDiffuse.rgb;
        ...
        // Fresnel effect (only applied to grass layer)
    float fresnelPwr = 4;
    float fresnelStrength = 1;
    float3 worldPos = IN.positionWS;
    float3 viewDir = normalize(_WorldSpaceCameraPos - worldPos);
    float3 normalWS = normalize(TransformObjectToWorldNormal(IN.normal));

    float fresnel = pow(1.0 - saturate(dot(normalWS, viewDir)), fresnelPwr);
    float3 fresnelColor = _FresnelColor.rgb * fresnel * fresnelStrength;

    // Layer mask: grass is splatControl.r
    float grassFactor = saturate(splatControl.r);

    // Blend fresnel only where grass is painted
    albedo = lerp(albedo, fresnelColor, fresnel * grassFactor);

    // Add a transition between the grass and dirt layers but make it fade to the dirt while keeping a hard edge to the grass
    // Identify your channels
    float edgeDarkness = 0.25;
    float fadeWidth = 0.5;
    float edgeWidth = 0.01;
    float dirt = splatControl.g;   // 1 for dirt pixels
    float grass = splatControl.r;  // 1 for grass pixels

    float2 uv = splatUV; // already calculated earlier

    // Texel size
    float2 texel = _Control_TexelSize.xy;
    float2 offset = texel * edgeWidth;

    // Sample 4 neighbors (grass channel this time)
    float gUp    = SAMPLE_TEXTURE2D(_Control, sampler_Control, uv + float2(0, offset.y)).r;
    float gDown  = SAMPLE_TEXTURE2D(_Control, sampler_Control, uv - float2(0, offset.y)).r;
    float gLeft  = SAMPLE_TEXTURE2D(_Control, sampler_Control, uv - float2(offset.x, 0)).r;
    float gRight = SAMPLE_TEXTURE2D(_Control, sampler_Control, uv + float2(offset.x, 0)).r;

    // Compute "proximity to grass" (0 = far from grass, 1 = directly at edge)
    float neighborGrass = max(max(gUp, gDown), max(gLeft, gRight));
    float proximity = saturate(neighborGrass - grass); // higher near transition
    ...
    if (dirt > 0.5)
    {
        // smooth falloff near grass edge
        fade = smoothstep(0.0, fadeWidth, proximity);
    }

    // Darken only dirt side, with smooth fade
    albedo = lerp(albedo, albedo * edgeDarkness, fade);
